...
import com.viktor44.rxswing3.sources.ChangeEventSource;
		...
		SwingTestHelper.create().runInEventDispatchThread(new Action() {
			...
			public void run() {
				...
				testSubscriber.assertValueAt(
						0, 
						new Predicate<ChangeEvent>() {
							@Override
							public boolean test(@NonNull ChangeEvent event) throws Throwable {
								return Objects.equals(tabbedPane, event.getSource());
							}
						}
				);
		...
		SwingTestHelper.create().runInEventDispatchThread(new Action() {
			...
			public void run() {
				try {
					...
					assertEquals("Class 'java.lang.String' has not the expected signature to support change listeners in com.viktor44.rxswing3.sources.ChangeEventSource",
							ex.getMessage());
				}
				...
				try {
					...
					assertEquals("Class '" + changeEventSource.getClass().getName() + "' has not the expected signature to support change listeners in com.viktor44.rxswing3.sources.ChangeEventSource",
							ex.getMessage());
				}
			}
		}).awaitTerminal();
...
//				assertEquals(1, onErrorEvents.size());
				...
				testSubscriber.assertError(
						new Predicate<Throwable>() {
							@Override
							public boolean test(@NonNull Throwable throwable) throws Throwable {
								assertEquals("Call of addChangeListener via reflection failed.", throwable.getMessage());
								assertEquals(InvocationTargetException.class, throwable.getCause().getClass());
								return true;
							}
						}
				);
